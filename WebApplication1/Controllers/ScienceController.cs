...
namespace WebApplication1.Controllers
{
    ...
    public class ScienceController : ControllerBase
    {
        ...
        public ScienceController(IConfiguration config)
        {
            _config = config;
        }
        ...
        public IActionResult AdminGetArticles([FromQuery] string? status = null)
        {
            ...
            var list = new List<ScienceStory>();
        ...
        public IActionResult DeleteArticle(int id)
        {
            ...
            var cmd = new MySqlCommand("DELETE FROM science_articles WHERE id = @Id", conn);
            cmd.Parameters.AddWithValue("@Id", id);
            cmd.ExecuteNonQuery();
        ...
        private static List<ScienceStory> QueryArticles(
        ...
        private static ScienceStory? GetArticleById(MySqlConnection conn, int id)
        {
            var cmd = new MySqlCommand(@"
                SELECT id, title, excerpt, author, read_time_minutes,
                       tone, is_pinned, pin_order, status, published_at, created_at, updated_at
                FROM science_articles WHERE id = @Id", conn);
            cmd.Parameters.AddWithValue("@Id", id);
            using var reader = cmd.ExecuteReader();
            return reader.Read() ? MapArticle(reader) : null;
        }

        private static void BindParams(MySqlCommand cmd, CreateScienceStoryRequest req, DateTime now)
        {
        ...
        private static ScienceStory MapArticle(MySqlDataReader r) => new ScienceStory
        {
            Id = r.GetInt32("id"),
